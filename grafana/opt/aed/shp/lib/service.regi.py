...
class Service:
    ...
    def __init__(self, service_name, service, use_configured_panel_ids):
        self.panels = []
        ...
        self.load_panels(service, use_configured_panel_ids)
        self.dashboard_uid = self.get_dashboard_uid(service)

    def get_dashboard_uid(self, service):
    ...
    def load_panels(self, service, use_configured_panel_ids):
        panels = service['panels']
        for panel in panels:
            my_panel = Panel(panels[panel], panel, use_configured_panel_ids, self.name)
    ...
    def __str__(self):
        s = ("Service Name: "      + self.name.encode('ascii', 'ignore') + '\n' +
             ...
             "  report grouping: " + self.report_grouping.encode('ascii', 'ignore')) + '\n'
