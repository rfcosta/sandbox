...
Generalize to HTTP API (Zabbix or Grafana)
...
__all__ = ('API', 'APIException', 'APIError')
__version__ = '1.2.4'
...
class APIException(Exception):
    ...
    def __init__(self, msg):
        super(APIException, self).__init__(hide_auth(msg))  # Remove sensitive information


class APIError(APIException):
    """
    Structured API error. Used for API errors.
...
class API(object):
    """
    Login and access any API method.
    """
    __username = None
    __password = None
    __auth = None
    ...
    def __getattr__(self, name):
        ...
        api_method = APISubClass(self, name)
    ...
    def do_request(self, json_obj):
        ...
        try:
            ...
            raise APIException('HTTP connection problem: %s' % e)
        ...
        if response.code != 200:
            raise APIException('HTTP error %s: %s' % (response.status, response.reason))

        reads = response.read()

        if len(reads) == 0:
            raise APIException('Received zero answer')

        try:
            jobj = json.loads(reads.decode('utf-8'))
        except ValueError as e:
            self.log(ERROR, 'Unable to decode. returned string: %s', reads)
            raise APIException('Unable to decode response: %s' % e)
        ...
        try:
            ...
            raise APIException('Missing result in API response')
    ...
    def login(self, user=None, password=None, save=True):
        ...
        if user and password:
            ...
            raise APIException('No authentication information available.')
    ...
    def relogin(self):
        ...
        try:
            ...
            self.log(ERROR, 'API relogin error (%s)', e)
    ...
    def check_auth(self):
        ...
        if not self.logged_in:
            if self.relogin_interval and self.last_login and (time() - self.last_login) > self.relogin_interval:
                self.log(WARNING, 'API not logged in. Performing API relogin after %d seconds',
                         self.relogin_interval)
                self.relogin()  # Will raise exception in case of login error
            else:
                raise APIException('Not logged in.')
    ...
    def call(self, method, params=None):
        ...
        start_time = time()
        self.check_auth()
        self.log(INFO, '[%s-%05d] Calling API method "%s"', start_time, self.id, method)
        ...
        try:
            ...
            self.log(INFO, '[%s-%05d] API method "%s" finished in %g seconds',
                     start_time, self.id, method, (time() - start_time))


class APISubClass(object):
    """
