...
def process_annotations_new(cls):
    ...
    for _dashboardId in _newAnnotations.keys():
        ...
        # hashcalc   = getattr(thismodule,'myCalcHash')
        # _existingAnnotations = cls.getAnnotations(HASH=hashcalc, DASHBOARDID=_dashboardId)
        _existingAnnotations = cls.getAnnotations_new(DASHBOARDID=_dashboardId)  # Hashes thru method on cls (cls.parseSysId)
        ...
        for _newAnnotation in _newAnnotations[_dashboardId]:
            ...
            if _dashboardId == d:
                ...
                if _rangesOnPanel:
                    shout("#process_annotations: new notation region: " + json.dumps(_newAnnotation))

                    _FOUND_SAME_RANGE = False
                    for _existingRange in _rangesOnPanel.keys():
                        shout("#process_annotations: Existing Region: " + str(_existingRange['regionId']))
