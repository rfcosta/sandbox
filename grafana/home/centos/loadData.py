...
parser = OptionParser()
parser.add_option("-data_file", dest="metrics_file", default="", help="Input data file",   required=False)
parser.add_option("-options",   dest="options_file", default="", help="Options json file", required=True)
parser.add_option("-service",   dest="service"     , default="", help="Service",           required=False)
parser.add_option("-key",       dest="key"         , default="", help="Key",               required=False)
parser.add_option("-metric",    dest="metric"      , default="", help="Metric",            required=False)
parser.add_option("-source",    dest="source"      , default="", help="Data Source",       required=False)
...
parameterNames = ["metrics_file", "service", "key", "metric", "source"]

for parm in parameterNames:
    value = options.get(parm)
    if value:
        OPTIONS[parm] = value
